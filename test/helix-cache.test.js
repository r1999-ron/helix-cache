...
test('SQLite persists registry, events, and measured cache/prefetch telemetry', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'helixcache-'));
  let cache;
  try {
    cache = await new HelixCache(root).init();
    await cache.register({ id: 'cold-model', content: 'adapter', tier: 'S3' });
    await cache.prefetch('use cold model');
    const first = cache.stats();
    assert.equal(first.measurements.prefetched, 1);
    assert.ok(first.measurements.wallClockLatencyMs >= 0);
    cache.close();
    cache = await new HelixCache(root).init();
    assert.equal(cache.list()[0].id, 'cold-model');
